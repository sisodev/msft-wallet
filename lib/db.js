...
export const insertUserDetails =  async (info) => {
  ...
  const db = await open({"filename": "./mydb.sqlite", driver: sqlite3.cached.Database})
  const statement = await db.prepare('INSERT INTO UserTbl(session_key, firstName, cooperateTitle, dateOfBirth, emailAddress, domicile, phoneNumber, privilegedSigner) values (?, ?, ?, ?, ?, ?, ?, ?)');
  const result = await statement.run(state, firstName, cooperateTitle, dateOfBirth, emailAddress, domicile, phoneNumber, privilegedSigner);
  return result
